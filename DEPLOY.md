...
- **새 글은 사라질 수 있음**: 무료 플랜은 디스크가 휘발성이라 서버가 잠들었다 깨면 DB가 초기화됩니다. 대신 시작할 때마다 **데모 데이터(test / 1234, 일기 232개)는 자동으로 다시 채워져서** 항상 보입니다. 시연용으론 충분합니다.
...
| 페이지는 뜨는데 로그인 안 됨 | test / 1234 로 시도. 안 되면 Render 로그(Logs 탭)에서 seed 실행됐는지 확인 |
