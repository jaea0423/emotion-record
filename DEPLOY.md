...
전체 흐름: ① Neon DB 만들기 → ② Cloudinary 만들기(사진) → ③ 로컬에서 동작 확인 → ④ GitHub 푸시 → ⑤ Render 환경변수 추가 → 끝.

알아둘 점
- **첫 접속이 느림**: Render 무료 플랜은 15분 미접속 시 잠들고, 다음 접속 때 깨는 데 ~1분 걸립니다. (제출 직전에 한 번 미리 접속해 깨워두면 교수님이 볼 때 바로 떠요.)
- **이제 모두 영구 저장**: 일기·계정·노래 링크는 Neon에, **업로드 사진은 Cloudinary**에 저장돼서 재시작해도 사라지지 않습니다. (단, Cloudinary `CLOUDINARY_URL`을 안 넣으면 사진은 로컬 폴더에 저장돼 배포 시 사라질 수 있어요.)
...
## 1.5단계 — Cloudinary 만들기 (사진 영구 저장, 무료, 5분)

1. https://cloudinary.com 접속 → **Sign up for free** (GitHub/구글 로그인 가능)
2. 가입 후 **Dashboard(대시보드)** 로 가면 **API Environment variable** 항목에 `CLOUDINARY_URL=cloudinary://...` 형태의 값이 있습니다. (눈 아이콘을 눌러 보이게 한 뒤) `cloudinary://` 부터 끝까지 **복사**하세요.
   - 형식: `cloudinary://<API_Key>:<API_Secret>@<Cloud_Name>`

이 값이 곧 **`CLOUDINARY_URL`** 입니다. API Secret이 들어 있으니 외부에 노출되면 안 되는 비밀값이에요.
...
1. 프로젝트 폴더(`C:\GitHub\emotion-record`)에서 새 라이브러리 설치:
   ...
   npm install
   ```
2. `.env` 파일을 열어 두 줄에 값을 채움 (1단계·1.5단계에서 복사한 값):
   ```
   DATABASE_URL=postgresql://...복사한_Neon_연결_문자열...
   CLOUDINARY_URL=cloudinary://...복사한_Cloudinary_값...
   ```
3. 더미 데이터 채우기 → 서버 실행:
   ```bash
   npm run seed
...
| `CLOUDINARY_URL` | 1.5단계에서 복사한 Cloudinary 값 (`cloudinary://...`) |

- `KAKAO_JS_KEY`, `GEMINI_API_KEY`는 이미 들어 있을 거예요(없으면 같이 넣기). `DATABASE_URL`도 이미 넣었으면 `CLOUDINARY_URL`만 추가하면 됩니다.
- 저장하면 Render가 자동으로 재배포합니다.

> 처음부터 새로 만드는 경우엔: New + → Blueprint → 레포 연결 → `render.yaml`이 자동으로 읽힘 → 4개 비밀값(`DATABASE_URL`/`CLOUDINARY_URL`/`KAKAO_JS_KEY`/`GEMINI_API_KEY`) 입력 → Apply.
